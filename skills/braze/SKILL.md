---
name: braze
description: Operate the Braze platform through braze-cli for category-first campaign, Canvas, segment, messaging, subscription, SMS, user, template, catalog, analytics, and content-block workflows. Use for bounded Braze reads, explicit opt-in or opt-out changes, structured JSON automation, permission diagnosis, and verified CLI delivery.
metadata:
  version: "1.2.0"
  binary: braze
---
...
3. Configure the CLI interactively:
   ...
   Copy the REST endpoint and API key from [**Settings > APIs and Identifiers > API Keys**](https://www.braze.com/docs/api/basics#endpoints) in Braze. The App ID is optional. Login masks the API key, writes the user config with mode `0600`, and never prints the key. Never print, copy, or commit credential values.
...
Inspect the exact leaf `--help` before constructing a request. It includes the detailed purpose, permission, authoritative Braze documentation, safe JSON input, executable command, and typed option constraints. Follow the linked Braze page when nested object semantics or provider behavior affect the request.
