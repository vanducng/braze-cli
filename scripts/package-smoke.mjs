...
try {
  ...
  const login = spawnSync(binary, ["login"], { cwd: prefix, env: cleanEnv, encoding: "utf8", input: "https://rest.example.com\npackage-login-secret\npackage-app\n" });
  assert.equal(login.status, 0, login.stderr);
  assert.doesNotMatch(login.stdout, /package-login-secret/u);
  assert.doesNotMatch(login.stderr, /package-login-secret/u);
  ...
  const localLogin = spawnSync(binary, ["login"], { cwd: prefix, env: cleanEnv, encoding: "utf8", input: `http://127.0.0.1:${server.address().port}\npackage-secret\n-\n` });
  assert.equal(localLogin.status, 0, localLogin.stderr);
  const success = await run(binary, ["campaign", "list"], { cwd: prefix, env: cleanEnv });
  assert.equal(success.status, 0, success.stderr);
  assert.equal(success.stderr, "");
  assert.deepEqual(JSON.parse(success.stdout), { campaigns: [] });
  status = 401;
  const failure = await run(binary, ["campaign", "list"], { cwd: prefix, env: cleanEnv });
